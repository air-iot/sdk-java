...
public class MQTTDataSender extends AbstractDataSender implements MqttCallbackExtended {
    ...
    private void connectTask() {
        ...
        while (true) {
            ...
            try {
                ...
                log.info("MQTTDataSender: 第 {} 次重连失败, 下次尝试时间[{}]", retryTimes,
    ...
    @Override
    public void doWriteLog(String deviceId, String level, String message) {
        if (!this.isRunning()) {
        ...
        MqttTopic tp = this.mqttClient.getTopic(String.format("logs/%s/%s/%s", this.projectId, level, deviceId));
        try {
            tp.publish(m);
        } catch (MqttException e) {
