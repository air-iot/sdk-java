...
import cn.airiot.sdk.driver.listener.Cmd;
import cn.airiot.sdk.driver.listener.DriverEventListener;
import cn.airiot.sdk.driver.listener.Result;
...
public class GrpcDriverEventListener implements DriverEventListener {
    ...
    private final DriverListenerProperties.Grpc grpcProperties;
    ...
    public GrpcDriverEventListener(DriverAppProperties driverProperties,
                                   DriverListenerProperties.Grpc grpcProperties,
    ...
    private void onConnected() {
        ManagedChannel channel = (ManagedChannel) this.driverGrpcClient.getChannel();
        ...
        StartHandler startHandler = new StartHandler(startCall, this.driverApp, this.globalContext,
                this.getDriverConfigType(), this::handleStreamClosed);
        startCall.start(startHandler, metadata);
        startCall.request(1);

        // schema
        ClientCall<SchemaResult, SchemaRequest> schemCall = channel.newCall(
                DriverServiceGrpc.getSchemaStreamMethod(),
                CallOptions.DEFAULT.withWaitForReady()
        );
        SchemaHandler schemaHandler = new SchemaHandler(schemCall, this.driverApp, this::handleStreamClosed);
        schemCall.start(schemaHandler, this.metadata);
        schemCall.request(Integer.MAX_VALUE);
    ...
    static class StartHandler extends ClientCall.Listener<StartRequest> {
        ...
        public StartHandler(ClientCall<StartResult, StartRequest> clientCall,
                            DriverApp<Object, Object, Object> driverApp,
                            GlobalContext globalContext, Type driverConfigType,
                            StreamClosedCallback closedCallback) {
            this.clientCall = clientCall;
            this.driverApp = driverApp;
            this.globalContext = globalContext;
        ...
        public void onMessage(StartRequest message) {
            ...
            try {
                ...
                this.globalContext.set(devices);
            } catch (Exception e) {
