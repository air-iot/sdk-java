...
public class WarningTests {
    ...
    void createWaring() {
        ...
        Response<InsertResult> response = this.warnClient.create(warning);
        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
        Assertions.assertNotNull(response.getData(), "创建报警失败");
    }

    @Test
    @Order(1)
    void queryById() {
        Response<Warning> response = this.warnClient.queryById("63fef49f09802fb164d52ed7", "false");

        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
        Assertions.assertNotNull(response.getData(), "未查询到报警信息");
    }

    @Test
    @Order(2)
    void queryAll() {
        Response<List<Warning>> response = this.warnClient.query(Query.newBuilder()
                .select(Warning.class)
                .build(), "false");

        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
        Assertions.assertNotNull(response.getData(), "未查询到报警信息");
        Assertions.assertFalse(response.getData().isEmpty(), "未查询到报警信息");
    }
}
