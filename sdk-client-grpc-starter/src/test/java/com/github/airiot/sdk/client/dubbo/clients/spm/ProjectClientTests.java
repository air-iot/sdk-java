...
import com.github.airiot.sdk.client.service.spm.dto.*;
...
public class ProjectClientTests {
    ...
    void createProject() {
        ...
        Response<ProjectCreateResult> response = this.projectClient.create(project);
    ...
    @Order(4)
    void queryById() {
        Response<Project> response = this.projectClient.queryById(this.projectId);
        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
        Assertions.assertNotNull(response.getData(), "未查询到项目信息: " + this.projectId);
    }
    ...
    void updateProjectBasicInfo() {
        ...
        Response<Void> response = this.projectClient.update(project);
        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
    }
    ...
    void updateProjectLicense() {
        ...
        Response<Void> response = this.projectClient.updateLicense(this.projectId, license);
        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
    }
    ...
    void replaceProject() {
        ...
        Response<Void> response = this.projectClient.replace(project);
        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
    }
    ...
    @Order(8)
    void deleteById() {
        Response<Void> response = this.projectClient.deleteById(this.projectId);
        Assertions.assertTrue(response.isSuccess(), response.getFullMessage());
    }
}
