...
import io.github.airiot.sdk.client.service.core.TableDataClient;
...
public class DubboTableDataClient implements TableDataClient {
    ...
    @Override
    public <T> ResponseDTO<InsertResult> create(@Nonnull String tableId, @Nonnull T row) {
        ...
        Response response = this.tableDataService.create(CreateDataRequest.newBuilder()
                .setTable(tableId)
                .setData(rowData)
                .build());
    ...
    @Override
    public <T> ResponseDTO<BatchInsertResult> create(@Nonnull String tableId, @Nonnull List<T> rows) {
        this.checkTableId(tableId);
    ...
    @Override
    public <T> ResponseDTO<Void> update(@Nonnull String tableId, @Nonnull String rowId, @Nonnull T data) {
        this.checkTableId(tableId);
        this.checkTableId(rowId);

        logger.debug("更新工作表记录: tableId = {}, rowId = {}, data = {}", tableId, rowId, data);

        Response response = this.tableDataService.update(UpdateDataRequest.newBuilder()
                .setTable(tableId)
                .setId(rowId)
    ...
    @Override
    public <T> ResponseDTO<Void> update(@Nonnull String tableId, @Nonnull Query query, @Nonnull T data) {
        this.checkTableId(tableId);
        ...
        Response response = this.tableDataService.updateMany(MultiUpdateDataRequest.newBuilder()
                .setTable(tableId)
    ...
    @Override
    public <T> ResponseDTO<Void> replace(@Nonnull String tableId, @Nonnull String rowId, @Nonnull T data) {
        this.checkTableId(tableId);
        this.checkTableId(rowId);

        logger.debug("替换工作表记录: tableId = {}, rowId = {}, data = {}", tableId, rowId, data);

        Response response = this.tableDataService.replace(UpdateDataRequest.newBuilder()
                .setTable(tableId)
                .setId(rowId)
                .setData(DubboClientUtils.serialize(data))
                .build());
    ...
    @Override
    public ResponseDTO<Void> deleteById(@Nonnull String tableId, @Nonnull String rowId) {
        this.checkTableId(tableId);
        this.checkRowId(rowId);

        logger.debug("删除工作表记录: tableId = {}, rowId = {}", tableId, rowId);

        Response response = this.tableDataService.delete(GetOrDeleteDataRequest.newBuilder()
                .setTable(tableId)
                .setId(rowId)
                .build());
    ...
    @Override
    public ResponseDTO<Void> deleteByQuery(@Nonnull String tableId, @Nonnull Query query) {
        this.checkTableId(tableId);
        ...
        Response response = this.tableDataService.deleteMany(QueryDataRequest.newBuilder()
                .setTable(tableId)
    ...
    @Override
    public <T> ResponseDTO<List<T>> query(@Nonnull Class<T> tClass, @Nonnull String tableId, @Nonnull Query query) {
        byte[] filter = query.serialize();
        if (logger.isDebugEnabled()) {
            logger.debug("查询工作表记录: tableId = {}, query = {}", tableId, new String(filter));
        }

        Response response = this.tableDataService.query(QueryDataRequest.newBuilder()
                .setTable(tableId)
                .setQuery(ByteString.copyFrom(filter))
                .build());
    ...
    @Override
    public <T> ResponseDTO<T> queryById(@Nonnull Class<T> tClass, @Nonnull String tableId, @Nonnull String rowId) {
        this.checkTableId(tableId);
        this.checkRowId(rowId);

        logger.debug("查询工作表记录: tableId = {}, rowId = {}, returnType = {}", tableId, rowId, tClass.getName());

        Response response = this.tableDataService.get(GetOrDeleteDataRequest.newBuilder()
                .setTable(tableId)
                .setId(rowId)
                .build());
