...
package io.github.airiot.sdk.client.dubbo.utils;
...
//            .setDateFormat("yyyy-MM-dd HH:mm:ss+HH:mm")
    ...
    public static <T> ResponseDTO<List<T>> deserializeList(Class<T> returnType, Response response) {
        List<T> result = null;
        ...
        return new ResponseDTO<>(response.getStatus(), response.getCount(), response.getCode(), response.getInfo(), response.getDetail(), result);
    }

    public static <T> ResponseDTO<T> deserialize(Class<T> returnType, Response response) {
        T result = null;
    ...
    public static <T> String format(ResponseDTO<T> responseDTO) {
