...
import io.github.airiot.sdk.client.dubbo.grpc.core.DubboTableSchemaServiceGrpc;
...
public class DubboTableSchemaClient implements TableSchemaClient {
    ...
    @Override
    public ResponseDTO<List<TableSchema>> query(@Nonnull Query query) {
        byte[] queryData = query.serialize();
        if (logger.isDebugEnabled()) {
            logger.debug("查询工作表定义: query = {}", new String(queryData));
        }

        Response response = this.tableSchemaService.query(
    ...
    @Override
    public ResponseDTO<List<TableSchema>> queryAll() {
    ...
    @Override
    public ResponseDTO<TableSchema> queryById(@Nonnull String tableId) {
        ...
        Response response = this.tableSchemaService.get(
